...
- **보안**: 구글 로그인 + 허용된 이메일 목록(`allowed_users` 테이블)으로 접근을 제한해요.
  `anon key`는 공개 저장소에 그대로 올라가 있지만, RLS 정책이 로그인한 사용자의 이메일이
  허용 목록에 있는지 확인하기 때문에 키만으로는 데이터를 읽거나 쓸 수 없어요. 앱 안의
  "👪 접근 관리" 화면에서 가족을 초대하거나 제거할 수 있어요.
